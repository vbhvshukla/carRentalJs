import { getItemsWithPagination, getAllItemsByIndex, getItemByKey, updateItem, deleteItem, getTotalItems } from "../utils/dbUtils.js";
        ...
        function renderBiddings(biddings) {
            ...
            const noBiddingsMessage = document.getElementById("no-biddings");

            tableBody.innerHTML = "";
            if(biddings.length===0){
                noBiddingsMessage.classList.remove("hidden");
            }
            else {
                noBiddingsMessage.classList.add("hidden");
            ...
            biddings.forEach(bid => {
                ...
                row.innerHTML = `
                    ...
                    <td><button class="chat-button" onclick="redirectToChat('${chatId}')">Chat</button>
                    ${bid.status.toLowerCase() === "cancelled" ? "" : `<button class="cancel-button" onclick="cancelBid('${bid.bidId}')">Cancel</button>`}</td>
                `;
